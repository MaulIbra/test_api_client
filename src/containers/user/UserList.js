    ...
    const listUser = users.map((value)=> {
        return(
            <tr className="d-flex" key={value.idUser}>
                <td className="col-3 border-top-0 border-right">{value.idCardNumber}</td>
                <td className="col-3 border-top-0 border-left-0 border-right">{value.username}</td>
                <td className="col-3 border-top-0 border-left-0 border-right">{Moment(value.dateOfBirth).format('LL')}</td>
